...
function NavDropdownMob(props) {
  ...
  return (
    <ul
      className="nav-dropdown-mob cursor-pointer"
      style={{ display: `${visState.visible ? "block" : "none"}` }}
    >
      <li className="nav-dropdown-item" onClick={onClick("ladder")}>
        Ladder
      </li>
      <li className="nav-dropdown-item" onClick={onClick("login")}>
        Login
      </li>
      <li className="nav-dropdown-item" onClick={onClick("register")}>
        Register
      </li>
    </ul>
  );
